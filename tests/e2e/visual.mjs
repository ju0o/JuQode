...
const results = await cdp(async ({ send, evalJs }) => {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    /* WCAG contrast of the primary button's own text on its own fill. The dark theme's
       --juq is a LIGHT teal, so white-on-it fails; this catches that class of regression. */
    out[`${theme}PriContrast`] = await evalJs(`(() => {
...
for (const theme of ['light', 'dark']) {
  const c = results[`${theme}PriContrast`];
  assert.ok(c >= 4.5, `${theme}: primary button contrast ${c}:1 is below AA (4.5:1)`);
}
