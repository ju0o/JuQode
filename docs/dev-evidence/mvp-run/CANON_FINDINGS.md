...
`19` §C4 는 실행 전에 카드가 **이해한 것 · 실행할 명령 · 하는 일**을 보여야 한다고 요구하고,
`18` 은 그 **라벨 세 개**(`qc.understood` · `qc.action` · `qc.meaning`)를 준다. 그런데 여섯 규칙
각각의 **문장**은 `18` 에 없다. 문장들은 Canon 자신의 기획 증거
(`../evidence/planning/q02-q03-quick-command-validation.md` §1)의 표에 있다.

구현은 그 표에서 그대로 옮겨 `gap:` 에 두었다 — 승인된 문구인 척하지 않는다. `18` 이 이 열여덟
문장(understood 6 · meaning 6 · 사용 불가 이유 6)을 받아야 한다.

두 번째, 작은 것: `15` TD-01 은 지원 동작 목록을 `실행 · 빌드/테스트 · 확인 · 터미널` 로 묶으라고
한다. 그런데 `실행` 은 `18` 에서 **실행 버튼**(`qc.run`)이다 — 같은 문자열이 두 가지를 뜻한다.
목록은 지금 평평하게 낸다. 제목을 위해 Canon 문구를 복제하지 않는다.
