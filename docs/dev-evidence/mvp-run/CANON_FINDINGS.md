...
## CF-22 · `15` TD-01 의 두 입력칸이 `15` 가 준 높이에 들어가지 않는다

`15` TD-01 은 **Inputs 를 둘** 요구한다 — Quick Command 자연어 칸과 셸 명령줄 — 그리고 같은
표에서 서랍 높이를 **290 px / 40 %** 로 고정한다. DV-11 이 파이프 셸로 판정되어 두 번째 입력칸이
실제로 생기자, 둘이 그 높이에 **들어가지 않는다**는 것이 실측으로 드러났다.

측정 (1200×770 창, 40vh = 308 px):

| 칸 | 필요 |
|---|---|
| 머리 + 상시 배너 | 86 px (닫을 수 없다 — `19` §S) |
| 셸 줄 (제목 · 한계 고지 · 출력 88 · 프롬프트) | 183 px |
| Quick Command (제목 · 입력 줄) | 98 px |
| **합** | **367 px** |

두 번의 스크린샷이 같은 것을 보여 줬다: 처음에는 **셸 프롬프트가 창 아래로 잘렸고**, 셸 줄을
고정한 뒤에는 **Quick Command 칸이 잘린 조각**이 됐다. 어느 쪽이든 `15` 가 요구한 입력칸 하나가
실질적으로 없어진다.

**택한 것:** 서랍을 `min(48vh, 460px)` 로 8vh 키운다. 두 입력칸을 다 보이게 하는 것이 높이
숫자보다 상위 요구라고 읽었다 — `15` 의 Primary Action 은 "type a command" 이고, 스크롤 아래로
밀린 Primary Action 은 Primary Action 이 아니다.

**택하지 않은 것:** 탭으로 하나씩 보이기(요구된 입력칸 하나를 숨긴다) · 셸 출력 무제한(프롬프트가
다시 밀린다) · Quick Command 접기(같은 문제를 반대편에서 만든다).

출력 상자는 여전히 **88 px 로 묶여 있다**(약 5줄, 나머지는 스크롤). 무제한으로 두면 서랍이
아무리 커도 프롬프트가 다시 밀린다.
