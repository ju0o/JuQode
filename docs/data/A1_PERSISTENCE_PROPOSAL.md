...
> | **Superseded By** | [`PERSISTENCE.md`](PERSISTENCE.md) — **현재 기준은 그쪽이다** |
...
> **A1b.1:** 런타임이 **Electron 44.x** 로 정해졌고 그 안의 **내장 `node:sqlite` 로 충분함이 실측**되었다.
> **바인딩 질문은 닫혔다.**
...
### ⚠ A1b.1 정정 — A1b 의 결론이 뒤집혔다

A1b 는 *"Electron 엔 `node:sqlite` 가 없다"* 고 적었다.
**그것은 Electron 탓이 아니라 Electron 33 이 번들한 Node 20 탓이었다.**
현재 안정판 **Electron 44.2.0 은 Node 24.20.0** 을 번들한다.

| 항목 | 시스템 Node 22 (A1a) | Electron 33 (A1b) | **Electron 44.2.0 (A1b.1)** |
|---|---|---|---|
| 번들 Node | — | 20.18.3 | **24.20.0** |
| `node:sqlite` | 있음 (experimental) | **없음** | **있다** |
| SQLite | 3.46.1 | — | **3.53.4** |
| WAL / 외래키 | ✔ / ✔ | — | **✔ / ✔** |
| JSON1 / 생성컬럼 인덱스 | ✔ / ✔ | — | **✔ / ✔** |
| FTS5 (MATCH 질의 확인) | ✔ | — | **✔** |
| `user_version` / `integrity_check` | ✔ | — | **✔ / `ok`** |
| **experimental 경고** | 발생 | — | **발생하지 않았다** |
| 네이티브 애드온 | 불필요 | **필요** | **불필요** |

> **정확한 버전:** Electron 44.2.0 · Chromium 152.0.7977.76 · Node 24.20.0 · SQLite 3.53.4

**PM 선호:** v0.1 은 **네이티브 애드온을 붙이지 말고 내장 `node:sqlite` 를 쓴다.**
**DB 접근은 하나의 로컬 영속성 서비스 뒤에 둔다. 플러그인 추상을 만들지 않는다.**
