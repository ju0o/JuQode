...
function qcRegion(api, state, repaint) {
  ...
  label.appendChild(el('span', 'kicker', C.qc.kicker));
  ...
  row.appendChild(btn('btn sm pri', C.intent.submit, route));
...
function routeCard(api, state, repaint) {
  ...
  /* `15` §0: every card that carries a claim names who acts. A Quick Command is JuQode's own —
   * `18` qc.notClaude says so in words, and the badge says it at a glance. */
  ...
  if (r.data?.command) {
    ...
    if (r.data.scriptBody) card.appendChild(el('p', 'xs mut mono', r.data.scriptBody));
  }
  ...
  acts.appendChild(btn('btn sm pri', C.qc.run, async () => {
    ...
    state.qcRun = started.ok ? { ...started, ruleId: id, state: 'running', tail: [] } : null;
    if (!started.ok) state.qcCard = { ...r, available: false, reason: started.detail ?? started.reason };
    repaint();
  }));
...
function runCard(api, state, repaint) {
  ...
  if (!r.ended) {
    ...
    acts.appendChild(btn('btn sm ghost rec', C.qc.rerun, async () => {
