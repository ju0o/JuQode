...
function availability(ruleId, { root, devServer = null } = {}) {
  ...
  if (ruleId === 'qc.git.status') {
    ...
    return yes({ command: 'git status --porcelain=v1 --branch', cwd: root, readOnly: true });
  }
  ...
  if (ruleId === 'qc.dev.start') {
    ...
    return yes({ command: `${pm} run ${script}`, script, scriptBody: scripts[script], cwd: root, pm });
  }

  if (ruleId === 'qc.build') {
    if (typeof scripts.build !== 'string' || !scripts.build.trim()) return no('no_script');
    return yes({ command: `${pm} run build`, script: 'build', scriptBody: scripts.build, cwd: root, pm });
  }
  ...
  if (ruleId === 'qc.test') {
    ...
    return yes({ command: `${pm} run test`, script: 'test', scriptBody: scripts.test, cwd: root, pm });
  }
